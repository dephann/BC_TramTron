...
namespace NDPSo.Chatbot
{
    public class OpenAIApiService
    {
        ...
        // GPT-4o: thông minh nhất, tốt nhất cho SQL generation
            ...
            if (!response.IsSuccessStatusCode)
            {
                try
                {
                    ...
                    if (errNode?.ContainsKey("message") == true)
                        throw new Exception($"OpenAI API lỗi: {errNode["message"]}");
                }
                catch (Exception ex) when (!ex.Message.StartsWith("OpenAI")) { }

                throw new Exception($"OpenAI API lỗi {(int)response.StatusCode}: {responseText}");
            }
        ...
        private string ParseResponse(string responseText, JavaScriptSerializer serializer)
        {
            ...
            {
                throw new Exception($"Lỗi parse OpenAI response: {ex.Message}");
            }
        }
    }
}